...
public class UsersControllerIntegrationTests : IClassFixture<TestWebApplicationFactory>
{
    ...
    private async Task<(User User, string Token)> SeedAdminAndLoginAsync()
    {
        ...
        using (var scope = _factory.Services.CreateScope())
        {
            ...
            // 2. Crear Permisos (Agregamos users.create)
            var readPerm = new Permission { Id = Guid.NewGuid(), Name = "users.read" };
            var createPerm = new Permission { Id = Guid.NewGuid(), Name = "users.create" }; // <-- Este es el que faltaba
    ...
    public async Task GetUsers_WithAdminToken_Returns200WithUserList()
    {
        ...
        content.Should().Contain("users").And.NotContain("PasswordHash");
    }
    ...
    [Fact]
    public async Task PostUser_WithAdminToken_Returns201WithUserId()
    {
        var setup = await SeedAdminAndLoginAsync();
        var newUser = new { Email = "new_integration@example.com", Password = "StrongPassword123!" };

        var request = new HttpRequestMessage(HttpMethod.Post, "/api/v1/users") { Content = JsonContent.Create(newUser) };
        request.Headers.Authorization = new AuthenticationHeaderValue("Bearer", setup.Token);
        var response = await _client.SendAsync(request);
